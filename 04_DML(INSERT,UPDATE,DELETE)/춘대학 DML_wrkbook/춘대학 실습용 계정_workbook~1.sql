...
WHERE STUDENT_NAME = '김명훈' AND S.DEPARTMENT_NO = (SELECT DEPARTMENT_NO,DEPARTMENT_NAME
FROM TB_DEPARTMENT) ;
...
UPDATE TB_GRADE
SET POINT = 3.5
WHERE (SELECT DISTINCT STUDENT_NO
FROM TB_GRADE
JOIN (SELECT STUDENT_NO
FROM TB_STUDENT
JOIN TB_CLASS USING (DEPARTMENT_NO)
JOIN TB_DEPARTMENT USING (DEPARTMENT_NO)
WHERE DEPARTMENT_NAME = '의학과' AND STUDENT_NAME = '김명훈') USING (STUDENT_NO)
WHERE TERM_NO = '200501' AND STUDENT_NO = 'A331101') = 'A331101';
 ROLLBACK;
