...
SELECT  EMP_NAME, SALARY, SALARY*12 , NVL((SALARY+SALARY*BONUS)*12,SALARY*12) AS "총수령액",
FLOOR(NVL((SALARY+SALARY*BONUS)*12 -((SALARY*12)*0.03),(SALARY*12)-(SALARY*12)*0.03)) AS "실수령액"
FROM EMPLOYEE;
...
SELECT EMP_NAME, SALARY, FLOOR(NVL((SALARY+SALARY*BONUS)*12 -((SALARY*12)*0.03),(SALARY*12)-(SALARY*12)*0.03)) AS "실수령액" , HIRE_DATE
FROM EMPLOYEE
WHERE FLOOR(NVL((SALARY+SALARY*BONUS)*12 -((SALARY*12)*0.03),(SALARY*12)-(SALARY*12)*0.03)) >=50000000;
...
--17. EMPLOYEE테이블에서 사원명, 입사일-오늘, 오늘-입사일 조회
 ...
 SELECT  EMP_NAME, ABS(FLOOR(HIRE_DATE - SYSDATE)) AS "근무일수1" , CEIL(SYSDATE - HIRE_DATE) AS "근무일수2"
 FROM EMPLOYEE;
...
SELECT EMP_NAME, DEPT_CODE, LPAD(EMP_NO,2) ||'년' || SUBSTR(EMP_NO,3,2)||'월' ||SUBSTR(EMP_NO,5,2) ||'일' AS "생년월일" ,
...
--23. EMPLOYEE테이블에서 사번이 201번인 사원명, 주민번호 앞자리, 주민번호 뒷자리,
 --주민번호 앞자리와 뒷자리의 합 조회
 SELECT EMP_NAME,SUBSTR(EMP_NO,1,6),SUBSTR(EMP_NO,8,7),
 TO_NUMBER(SUBSTR(EMP_NO,1,6))+TO_NUMBER(SUBSTR(EMP_NO,8,7)) AS "주민번호 합"
 FROM EMPLOYEE
 WHERE EMP_ID = '201';
...
--25. EMPLOYEE테이블에서 직원들의 입사일로부터 년도만 가지고 각 년도별 입사 인원수 조회
 ...
 SELECT COUNT(*),TO_CHAR(HIRE_DATE,'RRRR'), COUNT(HIRE_DATE) AS "입사인원수"
 FROM EMPLOYEE
 GROUP BY HIRE_DATE
 HAVING  TO_CHAR(HIRE_DATE,'RR') IN ('01','02','03','04');
